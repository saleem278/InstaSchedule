...
export class GroqTextProvider implements TextProvider {
  ...
  async generateFullContent(input: CaptionGenerationInput): Promise<CaptionGenerationOutput> {
    if (!config.GROQ_API_KEY) {
      throw new ExternalServiceError('Text generation is not configured. Set GROQ_API_KEY in your environment.');
    }
    ...
    try {
      ...
      throw new ExternalServiceError('Failed to generate content via Groq', { cause: error });
    }
  }
  ...
  async regenerateField(
    ...
    try {
      ...
      throw new ExternalServiceError('Failed to regenerate content via Groq', { cause: error });
    }
  }
}
